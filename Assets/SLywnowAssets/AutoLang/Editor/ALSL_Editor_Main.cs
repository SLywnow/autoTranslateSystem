		...
		*/
		...
		void OnGUI()
		{
			...
			if (nowtpe == tpe.editRK)
			{
				...
				{
					GUILayout.BeginHorizontal();
					EditorGUILayout.LabelField("<color=black> Key </color>", style);
					EditorGUILayout.LabelField("<color=black> Replace Word </color>", style);
					GUILayout.EndHorizontal();
			...
			if (nowtpe == tpe.addL)
			{
				...
				EditorGUILayout.LabelField("<color=black> Params: </color>", style);
				...
				EditorGUILayout.LabelField("<color=black> Now: " + inputAL + " </color>", style);
				scpos = GUILayout.BeginScrollView(scpos, style);
			...
			if (nowtpe == tpe.removeL)
			{
				...
				if (noweditid != -1)
				{
					EditorGUILayout.LabelField("<color=black>Removing " + ALSL_Editor_System.alllangs[noweditid] + ". Remove?</color>", style);
			...
			if (nowtpe == tpe.showL)
			{
				...
				else
				{
					...
					for (int i = 0; i < ALSL_Editor_System.alllangs.Count; i++)
					{
						...
						else EditorGUILayout.LabelField("<color=black>" + ALSL_Editor_System.alllangs[i] + "</color>", style);
			...
			if (nowtpe == tpe.editW)
			{
				...
				if (!(noweditid == -1 || noweditidl == -1))
				{
					...
					EditorGUILayout.LabelField("<color=black>key \"" + ALSL_Editor_System.keys_alsl[noweditidl] + "\" in " + ALSL_Editor_System.alllangs[noweditid] + "</color>", style);
			...
			if (nowtpe==tpe.paramdata)
			{
				if (paramreset)
				{
					...
					{
						inputpsd = ALSL_Editor_System.options.langpath;
				...
				GUILayout.BeginVertical();
				inputpsd = EditorGUILayout.TextField("Save Path", inputpsd);
				EditorGUILayout.LabelField("#dp - set default App page (Application.persistentDataPath)");
				EditorGUILayout.Space();
				...
				if (GUILayout.Button("Save"))
				{
					ALSL_Editor_System.options.langpath = inputpsd;
